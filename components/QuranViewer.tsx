...
const TAJWEED_CLASS_MAP: Record<string, string> = {
  // Madd
  madda_normal:      'madd-asli',
  madda_permissible: 'madd-fari',
  madda_obligatory:  'madd-fari',
  // Hamza
  ham_wasl:          'hamzat-wasl',
  // Idgham
  idgham_wo_ghunnah: 'idgham-no-gunnah',
  idgham:            'idgham-gunnah',
  // Ikhfa / Iqlab
  ikhafa:            'ikhfa',
  // Ghunnah
  ghunnah:           'ghunnah',
  ...
  qalaqah:           'qalqalah',
  // Laam
  laam_shamsiyah:    'laam-tafkheem',
  // Silent
  slnt:              'silent',
  // 🆕 Halaq + Mustaqilah - as-is pass karo
  halaq:             'halaq',
  mustaqilah:        'mustaqilah',
};

// ============================================
// SIMPLE TAJWEED FORMATTING - Keeps original classes
// ============================================

function formatTajweed(html: string): string {
  if (!html) return '';
  
  // Just wrap everything in a span with proper classes
  // No conversion needed - keep original classes
  
  // Ensure all tajweed spans have proper styling
  let formatted = html;
  
  // Add tajweed class to any span that has tajweed-related classes
  formatted = formatted.replace(
    /<span\s+class="([^"]*?(?:tajweed|ikhafa|qalaqah|ghunnah|halaq|mustaqilah|madda|laam|slnt|ham)[^"]*?)"/gi,
    (match, classes) => {
      // Keep original classes exactly as they are
          ...
          <div className="relative px-3 md:px-10 py-4" dir="rtl">
            {ayahs.map((ayah: Ayah) => {
              ...
              return (
                <div key={ayah._id} className="relative group mb-8">
                  {hasRuku && (<div className="ruku-container flex flex-col items-center"><span className="text-xs font-bold text-quran-gold">{ayah.ruku_surah || ''}</span><span className="text-2xl md:text-3xl font-bold text-quran-gold">ع</span><span className="text-xs font-bold text-quran-gold">{ayah.ruku_para || ''}</span></div>)}
                  {ayah.bismillah && (<div className="text-center my-3 text-lg md:text-2xl text-quran-bismillah dark:text-quran-bismillah-dark"><div dangerouslySetInnerHTML={{ __html: formatTajweed(ayah.bismillah) }} /></div>)}
                  <div className="relative">
                    <span className="quran-ayat-text font-bold leading-[2.4] text-quran-text dark:text-quran-text-dark" style={{ fontFamily: "'Scheherazade New', serif" }}><span dangerouslySetInnerHTML={{ __html: formatTajweed(ayah.textTajweed) }} /></span>
                    <span className="inline-block mr-2 text-quran-gold text-lg font-bold">({ayah.ayaIndex})</span>
                  </div>
                  <button onClick={() => handleMicClick(ayah)} disabled={isThisLoading && !isThisRecording} title={isThisRecording ? 'روکیں' : 'تلاوت شروع کریں'}
                    className={`absolute -right-10 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all duration-300 p-2 rounded-full shadow-lg hover:scale-110 ${isThisRecording ? 'bg-red-500 hover:bg-red-600 animate-pulse opacity-100' : 'bg-emerald-500 hover:bg-emerald-600'} disabled:opacity-40 disabled:cursor-not-allowed`}>
